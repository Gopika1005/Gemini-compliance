...
# Header
st.markdown('<h1 class="main-header">🛡️ Gemini Compliance Monitor</h1>', unsafe_allow_html=True)
st.markdown("### AI-Powered Real-time Regulatory Compliance Dashboard")

# Sidebar
with st.sidebar:
    st.image("https://www.google.com/images/branding/googlelogo/2x/googlelogo_color_92x30dp.png", width=150)
    st.markdown("---")
...
# Main Content
tab1, tab2, tab3, tab4 = st.tabs(["🏠 Dashboard", "🔍 Compliance Check", "📈 Analytics", "⚙️ Settings"])
...
with tab1:
    ...
    activity_data = {
        ...
        "Action": ["GDPR Audit Completed", "CCPA Violation Fixed", "AI Act Assessment", "New Regulation Added"],
        "Status": ["✅", "⚠️", "✅", "📥"]
    }
    ...
    deadlines = pd.DataFrame({
        "Regulation": ["AI Act Enforcement", "GDPR Amendments", "CCPA 2.0", "DMA Review"],
...
with tab2:
    ...
    with st.form("compliance_form"):
        ...
        with col1:
            ...
            user_count = st.number_input("Number of Users", min_value=1, value=10000, step=1000)
            revenue = st.number_input("Annual Revenue ($)", min_value=0, value=1000000, step=100000)
        
        with col2:
            data_collected = st.multiselect("Data Collected", 
                ["Email Address", "Phone Number", "Location", "Payment Info", 
                 "Health Data", "Biometric Data", "Usage Patterns", "Device Info"],
                default=["Email Address", "Location", "Usage Patterns"])
            
            ai_models = st.multiselect("AI Models Used",
                ["Gemini Pro", "GPT-4", "Custom ML", "Computer Vision", 
                 "Natural Language Processing", "Predictive Analytics", "Recommendation Engine"],
                default=["Gemini Pro", "Recommendation Engine"])
            
            storage_location = st.selectbox("Primary Data Storage",
                ["EU Only", "US Only", "Global", "EU-US Mix", "Asia", "Other"])
        
        regulations = st.multiselect("Regulations to Check",
            ["GDPR", "CCPA", "AI_ACT", "DMA", "HIPAA", "PIPEDA", "LGPD"],
            default=["GDPR", "CCPA", "AI_ACT"])
        
        priority = st.select_slider("Analysis Priority", 
    ...
    # Results Section
    if submitted or ('demo_report' in st.session_state and st.session_state.demo_report):
        ...
        with st.spinner("🤖 Analyzing compliance with AI..."):
            ...
            company_data = {
                ...
                "revenue": revenue,
                "processing_purposes": ["Personalization", "Analytics", "Service Delivery"],
                "industry": industry
            }
            ...
            try:
                ...
                with col3:
                    ...
                    # Fine breakdown
                    if estimated_fine:
                        st.metric("Percentage of Revenue", f"{(estimated_fine/revenue*100):.2f}%" if revenue else "N/A")
                ...
                if violations:
                    ...
                    styled_df = violations_df.style.applymap(
                        lambda x: color_severity(x) if x in ['critical', 'high', 'medium', 'low'] else '',
                        subset=['severity']
                    )
                    ...
                    if fixes:
                        for i, fix in enumerate(fixes, 1):
                            with st.expander(f"{i}. {fix.get('title', 'Fix')} - Priority: {fix.get('priority', 'medium').upper()}"):
                                ...
                                with col_b:
                                    col1, col2 = st.columns(2)
                                    with col1:
                                        st.metric("⏱️ Time", f"{fix.get('estimated_time_hours', 0)}h")
                                    with col2:
                                        st.metric("💰 Cost", f"${fix.get('cost_estimate_usd', 0):,}")
                                    ...
                                    st.write(f"**Impact:** {fix.get('compliance_impact', '')}")
                    else:
                        st.info("No specific fixes suggested. Consider consulting with compliance experts.")
                else:
                    st.success("🎉 No violations found! Your systems appear compliant.")
                
                # Audit Report
                st.subheader("📋 Audit Report")
                st.text_area("Detailed Report", results.get("audit_report", "No report generated"), height=300)
...
with tab3:
    ...
    with col1:
        ...
        fig = px.line(trend_data, x='Month', y=['GDPR', 'CCPA', 'AI_ACT'],
    ...
    with col2:
        ...
        violations_data = pd.DataFrame({
            'Type': ['Data Collection', 'User Consent', 'Security', 'Transparency', 'Documentation'],
            'Count': [45, 32, 28, 21, 15]
        })
        
        fig = px.bar(violations_data, x='Type', y='Count',
    ...
    fig = px.scatter(industry_data, x='Avg Score', y='Violations',
                    size='Violations', color='Industry',
                    hover_name='Industry', size_max=60,
...
with tab4:
    ...
    with col2:
        ...
        check_interval = st.select_slider("Check Interval", 
            ["1 hour", "6 hours", "12 hours", "Daily", "Weekly"], value="Daily")
